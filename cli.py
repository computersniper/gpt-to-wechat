...
def process_command(args):
    ...
    if args.copy_all and stickers:
        paths = [s.path for s in stickers if s.path and os.path.exists(s.path)]
        WeChatBridge.copy_files_to_clipboard(paths)
        console.print(f"[bold magenta]📋 全部 {len(paths)} 个表情已复制到系统剪贴板！可以直接在微信中按 Ctrl+V 一次性批量粘贴。[/bold magenta]")
    elif args.copy_first and stickers:
        WeChatBridge.copy_image_to_clipboard(stickers[0].path or stickers[0].image)
        console.print("[bold magenta]📋 表情 #01 已复制到系统剪贴板！可以直接在微信中 Ctrl+V 粘贴。[/bold magenta]")

    if args.push_wechat and stickers:
        paths = [s.path for s in stickers if s.path and os.path.exists(s.path)]
        console.print(f"[bold green]🚀 正在免鼠标批量推送全部 {len(paths)} 个表情到微信...[/bold green]")
        ok = WeChatBridge.paste_files_to_active_chat(paths)
        if ok:
            console.print("[bold green]✅ 成功将全部表情推送到微信输入区！在微信中按 Enter 发送即可！[/bold green]")
        else:
...
def main():
    ...
    p_proc.add_argument("--copy-all", action="store_true", help="处理后自动将全部表情一次性复制到剪贴板")
    p_proc.add_argument("--push-wechat", action="store_true", help="处理后免鼠标自动批量粘贴推送到微信")
